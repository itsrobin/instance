...
// *
// * 为GenericObjectPool和GenericKeyedObjectPool提供基本功能的类.这个类存在的主要原因是减少上述两个
// * 实现类的重复代码.
// * 这个类是线程安全的
// *
...
//    //配置属性
//    //可分配的对象的最大数量,默认为-1,为负说明对象池管理的对象数量没有上限.
//    private volatile int maxTotal =
//            GenericKeyedObjectPoolConfig.DEFAULT_MAX_TOTAL;
//    //资源耗尽后是否会阻塞,默认为true.
//    private volatile boolean blockWhenExhausted =
//            BaseObjectPoolConfig.DEFAULT_BLOCK_WHEN_EXHAUSTED;
//    //获取对象过程被阻塞后最大的等待时间,默认为-1,如果为负,那么线程在
//    //成功获取对象前将会无限期阻塞.
//    private volatile long maxWaitMillis =
//            BaseObjectPoolConfig.DEFAULT_MAX_WAIT_MILLIS;
//    //是否为后进先出队列,默认为true.
//    private volatile boolean lifo = BaseObjectPoolConfig.DEFAULT_LIFO;
//    //判断对象池对线程争用是否采取公平机制,默认为true,将会采用先进先出队列.
//    private final boolean fairness;
//    //在对象池创建对象到返回对象前是否应该对对象进行验证,默认为false
//    private volatile boolean testOnCreate =
//            BaseObjectPoolConfig.DEFAULT_TEST_ON_CREATE;
//    //在对象池返回对象前是否应该对对象进行验证,默认为false;
//    private volatile boolean testOnBorrow =
//            BaseObjectPoolConfig.DEFAULT_TEST_ON_BORROW;
//    //对象返回对象池之后是否应该对该对象进行验证,默认为false
//    private volatile boolean testOnReturn =
//            BaseObjectPoolConfig.DEFAULT_TEST_ON_RETURN;
//    //对象池中的空闲对象是否会被空闲对象清除者(evictor)清除,默认为false
//    private volatile boolean testWhileIdle =
//            BaseObjectPoolConfig.DEFAULT_TEST_WHILE_IDLE;
//    //清除者线程运行的时间间隔.如果不为正数,那么该线程不会启动,默认为-1
//    private volatile long timeBetweenEvictionRunsMillis =
//            BaseObjectPoolConfig.DEFAULT_TIME_BETWEEN_EVICTION_RUNS_MILLIS;
//    //空闲对象检测线程在每次执行时检测对象的最大数,默认为3
//    private volatile int numTestsPerEvictionRun =
//            BaseObjectPoolConfig.DEFAULT_NUM_TESTS_PER_EVICTION_RUN;
//    //对象可空闲的最小时间,超过这个时间可能就会被空闲对象清除者回收
//    private volatile long minEvictableIdleTimeMillis =
//            BaseObjectPoolConfig.DEFAULT_MIN_EVICTABLE_IDLE_TIME_MILLIS;
//    //对象可空闲的最小时间,超过这个时间可能就会被空闲对象清除者回收,除此之外,会保留minIdle数量的对象
...
//     *
//     * 返回对象池允许分配的对象的最大数量.如果是负数,那么说明对象池管理的对象数量没有上限
//     *
...
//     * 如果在对象池的对象已经耗尽时调用borrowObject()方法发生阻塞,返回true.反之为false;
...
//     *         will block.
//     * 在资源耗尽后,如果getBlockWhenExhausted返回true,调用borrowObject方法如果被阻塞.
//     * 那么将会等待{MaxWaitMillis}的时间,直到抛出异常或者超时.如果MaxWaitMillis<0,那么线程在
//     * 成功获取对象前将会无限期阻塞.
//     *
...
//     *
//     * 判断对象池对于空闲对象是否是后进先出-总是从对象池中返回最近被使用的对象,不然就是先进先出队列-
//     * 对象池总是返回空闲独享中最"老"的对象.
...
//     *在对象池创建对象后到borrowObject()方法返回对象前是否应该对对象进行验证.验证采用对象池工厂的validateObject()
//     * 方法进行验证,如果该对象验证失败,那么borrowObject方法将会失败.
//     * @see #setTestOnCreate
//     *
//     * @since 2.2
//     */
...
//     * 在borrowObject()方法返回对象前,是否应该对该对象进行验证,验证采用对象池工厂的validateObject()
//     * 方法进行验证.如果对象验证失败,那么这个对象将会从对象池中移除并摧毁,并且会再次从对象池中返回对象.
//     *
...
//     *
//     * 对象返回对象池之后是否应该对该对象进行验证.验证失败后该对象将会被销毁
//     * @see #setTestOnReturn
//     */
...
//     *
//     * 对象池中的空闲对象是否会被空闲对象清除者(evictor)清除.如果验证失败,将会移除该对象并销毁.
...
//     * will be run.
//     * 返回清除者线程运行的时间间隔.如果不为正数,那么该线程不会启动
...
//     * tested per run.
//     * 空闲对象检测线程在每次执行时检测对象的最大数.为正数时,会取配置的数值和对象池中的空闲对象数中
//     * 的最小值,作为检测的数量.如果为负数,那么n=ceil(getNumIdle()/abs(getNumTestsPerEvictionRun())),
//     * 那么1/n中的空闲对象将会在每次运行过程中被检测.
...
//     * 对象可空闲的最小时间,超过这个时间可能就会被空闲对象清除者回收.如果时间非正数,表示不会单单
//     * 更具空闲时间移除对象.
//     *
...
//     * 对象可空闲的最小时间,超过这个时间可能就会被空闲对象清除者回收,除此之外,会保留minIdle数量的对象.
//     * 他的功能会被MinEvictableIdleTimeMillis覆盖
//     *
//     * @return minimum amount of time an object may sit idle in the pool before
