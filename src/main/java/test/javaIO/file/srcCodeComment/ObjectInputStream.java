...
// * ObjectInputStream用于反序列化由ObjectOutputStream序列化对象产生的数据.
// *
...
// *
// * ObjectOutputStream 和 ObjectInputStream分别与FileOutputStream和FileInputStream一起使用,
// * 可以为应用提供对象持久化,
// * ObjectInputStream用于还原以前被序列化的对象.其他应用包括用于通过socket流在主机间传输对象,或者
// * 用于编组和解组远程通信系统中的实参和形参
// *
// *
// *
// *
...
// * ObjectInputStream确保从流中创建的图形中所有类型的对象和虚拟机中显示的类相匹配.
// * 使用标准机制按需加载类.
// *
// *
// * <p>Only objects that support the java.io.Serializable or
// * java.io.Externalizable interface can be read from streams.
// * 只有支持Serializable和Externalizable的接口的对象才能从流读取.
// *
// * <p>The method <code>readObject</code> is used to read an object from the
// * stream.  Java's safe casting should be used to get the desired type.  In
// * Java, strings and arrays are objects and are treated as objects during
// * serialization. When read they need to be cast to the expected type.
// * readObject方法被用于从流中读取对象.通过强制转换来获取所需的对象.在java中,字符和数组都是对象.
// * 读取的时候,需要将其进行类型转换
// *
// * <p>Primitive data types can be read from the stream using the appropriate
// * method on DataInput.
// * 可以使用DataInput的适当方法从流中获取基本数据类型
// *
...
// * 在默认情况下,java的反序列化机制会将对象的字段的内容恢复为它写入时的值和类型.静态和瞬态的字段将会
// * 被反序列化机制忽略.引用的其他对象也会从流中读取.使用引用共享机制将会帮助正确地恢复对象图形.在反
// * 序列化的时候,总是会分配一个新的对象以防现有的对象被重写.
// *
...
// * 读取一个对象和使用构造器创建对象是相似的.将会为这个对象分配内存.初始化为0(null),然后为不可序列化类
// * 执行无参构造方法.从最接近Object的类开始,从流中恢复可序列化的字段到特定的类型.
// *
// * <p>For example to read from a stream as written by the example in
// * ObjectOutputStream:
// * 举个例子:读取一个流,这个流时被ObjectOutputStream写入的
// * <br>
// * <pre>
...
// * 类控制serialized和Externalizable的实现.
// *
...
// * entire graphs.
// * 集成了Serializable接口的对象可以被保存,可以被恢复至完整状态.允许class在被写入流和从流读取的状态间变化.
// * 它自动遍历对象间的引用,保存和恢复全部图形
// *
// *
// *
...
// * methods:
// * 可序列化类如果想要在序列化和反序列化期间做特殊处理,必须实现下面的方法
// *
// * <pre>
...
// * readObject方法用来从流中读取并恢复特定对象的状态,而这个流时由对应的writeObject方法写入的.
// * 这个方法不需要涉及到对象的子类或者父类.会从ObjectInputStream中读取特定域的状态,并将其分配到对象
// * 的特定域中,DataInput可以帮助读取原生的数据类型.
// *
...
// * the allotted data.
// * 尝试读取由对应的writeObject写入的数据,如果超出数据自定义变价,将会抛出OptionalDataException(eof字段为true)
// * 如果非对象读取超出了已分配数据的末尾,将会以到达流的末尾的方式反应数据结束了.按位读取将会和字节读取或字节数读取
// * 一样返回-1,原生的读取将会抛出EOFExceptions.如果没有对应的writeObject方法,默认的序列化数据末尾标记已分配数据
// * 的末尾.
// *
// *
...
//     * methods.
//     * 创建一个ObjectInputStream并读取特定的输入流.序列化的流的头部会从流中读取并进行检查.
//     * 构造将会被阻塞,如果相对应的输出流将会写入并刷新head
//     *
//     *
//     *
//     * @param   in input stream to read from
//     * @throws  StreamCorruptedException if the stream header is incorrect
//     *          header如果header不准确,将会抛出StreamCorruptedException
//     * @throws  IOException if an I/O error occurs while reading stream header
//     *          在读取输入流头部时如果有I/O异常,将会抛出IOException
//     * @throws  SecurityException if untrusted subclass illegally overrides
//     *          security-sensitive methods
//     *          如果不可信的子类非法覆盖了安全敏感的方法
...
//     * 从ObjectInputStream中读取一个对象.对象的类,对象的签名,对象自身和超类非顺态和静态域的值都将
//     * 被读取.通过重写writeObject和readObject方法可以重新制定反序列化的规则
//     *
...
//     * 验证这个实例(或者它的父类)在可以被构造的情况下没有违反安全限制:.1.父类绝对不能覆盖和安全相关不可
//     * 更改的方法,2.SerializablePermission具有enableSubclassImplementation权限
//     */
...
//     * 读取空变量,保存空的句柄至passHandle并返回null
//     */
...
//     * This class implements an exception-propagation algorithm_1_2 for
...
//     * algorithm_1_2 to aggressively prune dependency links, lessening the
//     * performance/memory impact of exception tracking.
//     *
//     * <p>Note that the exception propagation algorithm_1_2 used depends on handles
//     * being assigned/finished in LIFO order; however, for simplicity as well
//     * as memory conservation, it does not enforce this constraint.
//     */
//    // REMIND: add full description of exception propagation algorithm_1_2?
